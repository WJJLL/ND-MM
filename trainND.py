...
from config import  IMAGENET_Train_PATH, IMAGENET_Test_PATH
...
def parse_arguments():
    ...
    parser.add_argument('--t', type=int, default=1, help='temperature T')
    ...
    parser.add_argument('--num_models', type=int, default=4, help='Number of training models')
...
def train(train_loader,model_pool, noise, epoch, optimizer,criterion,args):
    ...
    for i, (input,label) in enumerate(train_loader):
        ...
        for ll in logits_set:
            if args.match_target!=-1:
                loss_kl += ND_loss(ll, logits, target.cuda(), alpha=0, beta=1, temperature=1)
            else:
                loss_kl += ND_loss(ll, logits, logits.argmax(dim=-1).detach(), alpha=0, beta=1, temperature=1)
        loss = loss_fuse + loss_kl
